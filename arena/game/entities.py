...
class Player:
    ...
    def update_style_1(self, action):
        ...
        if action == 1:  # Thrust
            thrust_vec = utils.vector_from_angle(
                self.rotation, config.PLAYER_THRUST)
        ...
        self.velocity = utils.limit_magnitude(
            self.velocity, config.PLAYER_MAX_VELOCITY)
    ...
    def heal(self, percent):
        ...
        heal_amount = self.max_health * percent
...
class Spawner:
    ...
    def __init__(self, x, y, spawn_rate_multiplier=1.0):
        ...
        self.spawn_rate = int(
            config.SPAWNER_SPAWN_COOLDOWN * spawn_rate_multiplier)
        self.alive = True
    ...
    def spawn_enemy(self, np_random=np.random, speed_multiplier=1.0, existing_enemies=None):
        """Create a new enemy around the spawner, ensuring distance from others."""
        if self.spawn_cooldown == 0:
            self.spawn_cooldown = self.spawn_rate
            
            # Try multiple positions to find one not too crowded
            best_pos = None
            min_dist_required = config.MIN_ENEMY_SPAWN_DISTANCE
            
            for _ in range(5):
                angle = np_random.random() * 2 * math.pi
                dist = self.radius + config.ENEMY_RADIUS + np_random.uniform(5, 50)
                offset = utils.vector_from_angle(angle, dist)
                ...
                if existing_enemies:
                    too_close = any(utils.distance(spawn_pos, e.pos) < min_dist_required 
                                  for e in existing_enemies if e.alive)
                    if too_close:
                        continue
            ...
            # If no good position found after retries, use the last one generated or fallback
            if best_pos is None:
                angle = np_random.random() * 2 * math.pi
                offset = utils.vector_from_angle(angle, self.radius + config.ENEMY_RADIUS + 5)
                best_pos = self.pos + offset
                best_pos = utils.keep_in_bounds(
                    best_pos, 0, config.GAME_WIDTH, 0, config.GAME_HEIGHT, config.ENEMY_RADIUS
                )

            self.enemies_spawned += 1
            return Enemy(best_pos[0], best_pos[1], speed_multiplier)
        return None
