...
# Phase Progression: Critical Milestones
...
# Quick Kill Bonus: Phase-Aware (implemented in environment)
REWARD_QUICK_KILL_BASE = 20.0  # Base quick-kill reward
REWARD_QUICK_KILL_PHASE_MULTIPLIERS = [1.0, 1.2, 1.5, 2.0, 3.0]  # Per phase scaling
REWARD_QUICK_KILL_TIME_THRESHOLD = 500  # Steps threshold for "quick"

# Health Management: Explicit survival incentives (NEW!)
REWARD_HEALTH_THRESHOLD_HIGH = 5.0  # Bonus if completing phase with >80% HP
REWARD_HEALTH_THRESHOLD_MED = 2.0  # Bonus if completing phase with >50% HP
REWARD_HEALTH_HIGH_THRESHOLD = 0.8  # 80% health threshold
REWARD_HEALTH_MED_THRESHOLD = 0.5  # 50% health threshold

# Penalties: Stronger avoidance signals
REWARD_DEATH = -200.0  # Make death very costly (was -100)
REWARD_DAMAGE_TAKEN = -5.0  # Stronger damage avoidance (was -3)
REWARD_STEP_SURVIVAL = -0.02  # Moderate time pressure (was -0.01)
...
class TrainerConfig:
    ...
    ppo_net_arch: Dict[str, List[int]] = field(
        default_factory=lambda: dict(
            pi=[512, 384, 256, 128, 64], vf=[512, 384, 256, 128, 64]
        )
    )
