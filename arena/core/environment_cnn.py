...
class ArenaCNNEnv(gym.Env):
    ...
    def _compute_wall_field(self):
        ...
        for y in range(HEATMAP_SIZE):
            for x in range(HEATMAP_SIZE):
                ...
                field[y, x] = 1.0 - min(min_dist * 4.0, 1.0)  # Steep gradient near edges
    ...
    def _add_gaussian_blob(self, channel, world_x, world_y, intensity=1.0, sigma=GAUSSIAN_SIGMA):
    ...
    def _render_heatmap(self):
        ...
        if self.player and self.player.alive:
            self._add_gaussian_blob(heatmap[0], self.player.pos[0], self.player.pos[1], 
                                   intensity=1.0, sigma=GAUSSIAN_SIGMA)
        ...
        for enemy in self.enemies:
            if enemy.alive:
                dist = utils.distance(self.player.pos, enemy.pos) if self.player else max_dist
                threat = 1.0 - (dist / max_dist)  # Closer = higher threat
                self._add_gaussian_blob(heatmap[1], enemy.pos[0], enemy.pos[1], 
                                       intensity=max(0.3, threat), sigma=GAUSSIAN_SIGMA * 1.2)
        
        # Channel 2: Spawner positions (intensity = health ratio)
        for spawner in self.spawners:
            if spawner.alive:
                health_ratio = spawner.health / spawner.max_health
                self._add_gaussian_blob(heatmap[2], spawner.pos[0], spawner.pos[1], 
                                       intensity=health_ratio, sigma=GAUSSIAN_SIGMA * 1.5)
        ...
        for proj in self.projectiles:
            if proj.alive and not proj.is_player_projectile:
                if self.player:
                    ...
                    danger = 0.5
                self._add_gaussian_blob(heatmap[3], proj.pos[0], proj.pos[1], 
                                       intensity=max(0.5, danger), sigma=GAUSSIAN_SIGMA * 0.8)
    ...
    def _get_scalar_features(self):
        """Get auxiliary scalar features."""
        scalars = np.zeros(SCALAR_DIM, dtype=np.float32)
        
        if self.player:
            # [0] Player health ratio
            scalars[0] = self.player.get_health_ratio()
            
            # [1] Shoot cooldown ratio
            scalars[1] = self.player.shoot_cooldown / config.PLAYER_SHOOT_COOLDOWN
            
            # [2] Phase progress ratio
            scalars[2] = self.current_phase / config.MAX_PHASES
            
            # [3] Time remaining ratio
            scalars[3] = 1.0 - (self.current_step / config.MAX_STEPS)
            
            # [4] Enemy count (normalized)
            scalars[4] = len([e for e in self.enemies if e.alive]) / config.SPAWNER_MAX_ENEMIES
            
            # [5] Spawners remaining ratio
            phase_idx = min(self.current_phase, config.MAX_PHASES - 1)
            initial_spawners = config.PHASE_CONFIG[phase_idx]['spawners']
            scalars[5] = len([s for s in self.spawners if s.alive]) / max(initial_spawners, 1)
            
            # [6] Player velocity magnitude (normalized)
            vel_mag = math.sqrt(self.player.velocity[0]**2 + self.player.velocity[1]**2)
            scalars[6] = np.clip(vel_mag / config.PLAYER_MAX_VELOCITY, 0, 1)
