...
class BaseTrainer(ABC):
    ...
    def __init__(self, config: TrainerConfig):
        ...
        self.run_dir = os.path.join(
            config.runs_dir, config.algo, f"style{config.style}", self.run_name
        )
        ...
        if arena_config.CURRICULUM_ENABLED:
            self.curriculum_manager = CurriculumManager(CurriculumConfig(enabled=True))
    ...
    def create_environment(self) -> None:
        ...
        if self.device == "mps" and num_envs > 1:
            print(
                f"Applying MPS multiprocessing workaround: using CPU for env workers."
            )
        ...
        if num_envs > 1 and not self.config.render:
            try:
                ...
                print(
                    f"Failed to create SubprocVecEnv: {e}. Falling back to DummyVecEnv."
                )
    ...
    def _find_vecnormalize_stats(self, model_path: str) -> Optional[str]:
        ...
        if run_prefix:
            ...
            for subdir in ["checkpoints", "final", "."]:
                search_dir = (
                    os.path.join(parent_dir, subdir) if subdir != "." else parent_dir
                )
    ...
    def _load_pretrained_model(
        ...
        if self.config.load_vecnormalize:
            ...
            if vecnorm_path and os.path.exists(vecnorm_path):
                ...
                unwrapped_env = (
                    self.env.venv if isinstance(self.env, VecNormalize) else self.env
                )
                self.env = VecNormalize.load(vecnorm_path, unwrapped_env)
                print(
                    "✓ VecNormalize stats applied successfully (continuing with pretrained normalization)"
                )
            else:
                print(
                    "⚠ No VecNormalize stats found, using fresh normalization (stats will reset)"
                )
        ...
        if self.config.load_curriculum and self.curriculum_manager:
            ...
            if training_state:
                ...
                print(
                    f"Restored curriculum state: stage {self.curriculum_manager.current_stage_index} "
    ...
    def _pretrain_with_behavioral_cloning(self):
        """Pretrain the model using behavioral cloning from demonstrations."""
        from arena.training.imitation_learning import pretrain_from_demonstrations

        if not self.config.demo_path:
            return

        if not os.path.exists(self.config.demo_path):
            print(f"Warning: Demonstration file not found: {self.config.demo_path}")
            return

        print("\n" + "=" * 60)
        print("BEHAVIORAL CLONING PRETRAINING")
        print("=" * 60)
        print(f"Demo file: {self.config.demo_path}")
        print(f"Epochs: {self.config.bc_epochs}")
        print(f"Learning rate: {self.config.bc_learning_rate}")
        ...
        try:
            history = pretrain_from_demonstrations(
                model=self.model,
                demo_path=self.config.demo_path,
            ...
            print("\n" + "=" * 60)
            print("BEHAVIORAL CLONING PRETRAINING COMPLETE")
            print(f"Final Loss: {history['loss'][-1]:.4f}")
            print(f"Final Accuracy: {history['accuracy'][-1] * 100:.2f}%")
            print("=" * 60 + "\n")

        except Exception as e:
            print(f"Error during behavioral cloning pretraining: {e}")
            print("Continuing with RL training...")
    ...
    def train(self) -> BaseAlgorithm:
        ...
        if self.config.pretrained_model_path:
            ...
            # Behavioral Cloning pretraining (if enabled)
            if self.config.bc_pretrain and self.config.demo_path:
                self._pretrain_with_behavioral_cloning()
    ...
    def save_final_model(self) -> str:
        ...
        if isinstance(self.env, VecNormalize):
            vecnorm_path = save_path.replace(".zip", "").replace(
                f"{self.run_name}_final", f"{self.run_name}_vecnormalize_final.pkl"
            )
