...
class Evaluator:
    ...
    def run_session(
        ...
        if vecnorm_path and os.path.exists(vecnorm_path):
            ...
            print(
                "⚠ This is expected for old models but indicates a bug for new models"
            )
            self.env = vec_env
    ...
    def run_recording_session(self, style: int, save_path: Optional[str] = None):
        """Run a recording session to collect human demonstrations."""
        if self.env:
            self.env.close()
        ...
        # Create metrics object for the renderer
        metrics = {
            "episode": 0,
            "episode_reward": 0.0,
            "total_reward": 0.0,
        ...
        print("\n" + "=" * 60)
        print("DEMONSTRATION RECORDING MODE")
        print("=" * 60)
        print("Play the game normally. Each episode will be recorded.")
        print("Press ESC to stop recording and save demonstrations.")
        print("=" * 60 + "\n")
            ...
            for event in events:
                if event.type == pygame.QUIT:
                    ...
                    if len(recorder.buffer) > 0:
                        filepath = recorder.save_demonstrations(save_path)
                        print(f"\nSaved to: {filepath}")
                    return "quit"
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                    ...
                    if action == "start":
                        ...
                        if selection:
                            if selection["mode"] == "Human Player":
                                state = self.run_human_session(selection["style"])
                            elif selection["mode"] == "Record Demos":
                                state = self.run_recording_session(selection["style"])
                            else:
                            ...
                            if (
                                self.menu.gameplay_modes[self.menu.selected_mode_idx]
                                == "Model"
                            ):
                                ...
                                self.menu.set_status("No selection made", "warning")
