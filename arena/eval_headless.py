...
class HeadlessEvaluator:
    ...
    def _create_env(self, style: int, algo: str, vecnorm_path: Optional[str] = None,
        ...
        if curriculum_stage is not None:
            ...
            self._log(
                f"[OK] Using curriculum stage {curriculum_stage}: {curriculum_manager.current_stage.name}")
        ...
        if num_envs > 1:
            ...
            self._log(
                f"[OK] Created {num_envs} parallel environments (SubprocVecEnv)")
        else:
    ...
    def run_episode(
        ...
        while not done:
            ...
            if done:
                ...
                first_spawner_kill_step = info.get(
                    'first_spawner_kill_step', -1)
            ...
            for i, (done, info) in enumerate(zip(dones, infos)):
                if done and len(completed_episodes) < num_episodes:
                    episode_stats = EpisodeStats(
                        ...
                        win_step=info.get('win_step', -1),
                        phase_reached=info.get('phase', 0),
                        spawners_destroyed=info.get(
                            'total_spawners_destroyed', 0),
                        enemies_destroyed=info.get(
                    ...
                    if len(completed_episodes) % max(1, num_episodes // 10) == 0:
                        self._log(
                            f"  Progress: {len(completed_episodes)}/{num_episodes} episodes ({len(completed_episodes)/num_episodes*100:.1f}%)")
    ...
    def evaluate_model(
        ...
        self._log(
            f"Episodes: {num_episodes} | Style: {style} | Deterministic: {deterministic} | Workers: {num_workers}")
        self._log(f"{'='*80}")
        ...
        if auto_curriculum and curriculum_stage is None:
            ...
            if training_state:
                ...
                self._log(
                    f"[OK] Auto-detected curriculum stage: {effective_curriculum_stage}")
            else:
                self._log(
                    "⚠ No training state found, using full difficulty (no curriculum)")

        # Find and load VecNormalize stats
        vecnorm_path = self._find_vecnormalize_stats(model_path)
        if vecnorm_path:
            self._log(
                f"[OK] Found VecNormalize stats: {os.path.basename(vecnorm_path)}")
        else:
            self._log("⚠ WARNING: No VecNormalize stats found!")

        # Create environment with curriculum stage if specified
        env, has_vecnorm = self._create_env(
            style, algo, vecnorm_path, effective_curriculum_stage, num_workers)
        ...
        if num_workers > 1:
            ...
            for i in range(num_episodes):
                episode_stats = self.run_episode(
                    model, env, is_recurrent, deterministic)
                episodes.append(episode_stats)

                # Progress indicator
                if (i + 1) % max(1, num_episodes // 10) == 0:
                    self._log(
                        f"  Progress: {i+1}/{num_episodes} episodes ({(i+1)/num_episodes*100:.1f}%)")
    ...
    def _compute_statistics(
        ...
        first_spawner_kills = np.array(
            [e.first_spawner_kill_step for e in episodes if e.first_spawner_kill_step > 0])

        # Compute distributions
        phase_dist = {int(p): int(np.sum(phases == p))
                      for p in range(int(phases.max()) + 1)}
        spawner_dist = {int(s): int(np.sum(spawners == s))
                        for s in range(int(spawners.max()) + 1)}
        ...
        return ModelEvaluation(
            ...
            # Timing metrics
            mean_win_step=float(win_steps.mean()) if len(
                win_steps) > 0 else -1.0,
            mean_first_spawner_kill=float(first_spawner_kills.mean()) if len(
                first_spawner_kills) > 0 else -1.0,
...
def print_evaluation_summary(eval_result: ModelEvaluation):
    ...
    print(f"{'='*80}")
    print(
        f"Algorithm: {eval_result.algorithm} | Style: {eval_result.style} | Deterministic: {eval_result.deterministic}")
    print(
        f"Episodes: {eval_result.total_episodes} | Eval Time: {eval_result.eval_time_seconds:.2f}s")
    print(
        f"Speed: {eval_result.total_episodes / eval_result.eval_time_seconds:.1f} episodes/sec")

    print(f"\n{'-'*80}")
    print("WIN STATISTICS")
    print(f"{'-'*80}")
    print(
        f"Win Rate:        {eval_result.win_rate*100:6.2f}% ({eval_result.wins}/{eval_result.total_episodes})")
    ...
    print(f"{'-'*80}")
    print(
        f"Mean:            {eval_result.mean_reward:8.2f} ± {eval_result.std_reward:.2f}")
    print(f"Median:          {eval_result.median_reward:8.2f}")
    print(
        f"Min/Max:         {eval_result.min_reward:8.2f} / {eval_result.max_reward:.2f}")

    print(f"\n{'-'*80}")
    print("EPISODE LENGTH")
    print(f"{'-'*80}")
    print(
        f"Mean:            {eval_result.mean_length:6.0f} ± {eval_result.std_length:.0f} steps")
    print(f"Median:          {eval_result.median_length:6.0f} steps")
    print(
        f"Min/Max:         {eval_result.min_length:6d} / {eval_result.max_length:d} steps")

    print(f"\n{'-'*80}")
    print("PERFORMANCE METRICS")
    print(f"{'-'*80}")
    print(
        f"Spawners/Episode:{eval_result.mean_spawners_destroyed:6.2f} ± {eval_result.std_spawners_destroyed:.2f}")
    ...
    if eval_result.mean_first_spawner_kill > 0:
        print(
            f"1st Spawner Kill:{eval_result.mean_first_spawner_kill:6.0f} steps")
...
def print_comparison_table(evaluations: List[ModelEvaluation]):
    ...
    print(
        f"Best Win Rate:      {best_winrate.model_name} ({best_winrate.win_rate*100:.2f}%)")

    best_reward = max(evaluations, key=lambda e: e.mean_reward)
    print(
        f"Best Mean Reward:   {best_reward.model_name} ({best_reward.mean_reward:.2f})")

    best_spawners = max(evaluations, key=lambda e: e.mean_spawners_destroyed)
    print(
        f"Best Spawner Kills: {best_spawners.model_name} ({best_spawners.mean_spawners_destroyed:.2f})")

    best_phase = max(evaluations, key=lambda e: e.mean_phase_reached)
    print(
        f"Best Phase:         {best_phase.model_name} ({best_phase.mean_phase_reached:.2f})")
...
def save_results_csv(evaluations: List[ModelEvaluation], output_path: str):
    ...
    metrics = [
        ...
        ('Speed (eps/sec)',
         lambda e: f"{e.total_episodes / e.eval_time_seconds:.1f}"),
        ('Win Rate (%)', lambda e: f"{e.win_rate * 100:.1f}"),
        ('Avg Win Step',
         lambda e: f"{int(e.mean_win_step)}" if e.mean_win_step > 0 else "N/A"),
        ...
        ('Mean Episode Length',
         lambda e: f"{int(e.mean_length)} ± {int(e.std_length)}"),
        ...
        ('Spawners / Episode',
         lambda e: f"{e.mean_spawners_destroyed:.2f} ± {e.std_spawners_destroyed:.2f}"),
        ...
        ('First Spawner Kill (steps)',
         lambda e: f"{int(e.mean_first_spawner_kill)}" if e.mean_first_spawner_kill > 0 else "N/A"),
    ]

    # Add phase distribution metrics
    max_phase = max(max(e.phase_distribution.keys())
                    for e in evaluations) if evaluations else 0
    ...
    max_spawners = max(max(e.spawner_kill_distribution.keys())
                       for e in evaluations) if evaluations else 0
...
def main():
    ...
    model_group.add_argument(
        '--model', type=str, help='Path to single model file')
    model_group.add_argument('--models', nargs='+',
                             help='Paths to multiple model files')
    model_group.add_argument('--directory', type=str,
                             help='Directory to search for models')

    # Evaluation parameters
    parser.add_argument('--episodes', type=int, default=100,
                        help='Number of episodes per model (default: 100)')
    parser.add_argument('--style', type=int, default=None, choices=[
                        1, 2], help='Control style (auto-detected from model path if not specified)')
    ...
    parser.add_argument('--csv', type=str, default=None,
                        help='Output CSV file path for tabular results')
    parser.add_argument('--save-episodes', action='store_true',
                        help='Save raw episode data in output')
    parser.add_argument('--quiet', action='store_true',
                        help='Suppress progress messages')
    parser.add_argument('--compare', action='store_true',
                        help='Print comparison table for multiple models')
